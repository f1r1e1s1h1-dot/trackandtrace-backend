...
# --------------------------------------------------------
# GPS → CITY USING OPENSTREETMAP
# --------------------------------------------------------
def get_city_from_gps(lat, lng):
    try:
        url = f"https://nominatim.openstreetmap.org/reverse?lat={lat}&lon={lng}&format=json"
        r = requests.get(url, headers={"User-Agent": "TrackAndTraceApp"})
        data = r.json()
        return (
...
# --------------------------------------------------------
# QR ROOT RESOLVER
# Walk backwards using QRLink to find RollMaster.qr_value
# --------------------------------------------------------
def resolve_root_qr(qr_value: str) -> str:
    ...
    while True:
        ...
        link = QRLink.objects.filter(current_qr_value=current).order_by("-id").first()
        if not link:
            break
...
def get_all_qrs_from_root(root_qr: str):
    ...
    while frontier:
        next_frontier = set()
        links = QRLink.objects.filter(previous_qr_value__in=list(frontier)).values(
            "previous_qr_value", "current_qr_value"
        )
        for l in links:
...
# --------------------------------------------------------
# Helpers: consistent phase response format
# --------------------------------------------------------
...
def build_roll_process_response(roll: RollMaster, qr_used: str):
    """
    Builds the same response used by /timeline and admin search.
    """
    ...
    dispatch_obj = Dispatch.objects.filter(qr_value__in=list(all_qrs)).order_by("-id").first()
    delivery_obj = DeliveryConfirmation.objects.filter(qr_value__in=list(all_qrs)).order_by("-id").first()

    delivery_city = None
    if delivery_obj and getattr(delivery_obj, "gps_lat", None) and getattr(delivery_obj, "gps_lng", None):
        delivery_city = get_city_from_gps(delivery_obj.gps_lat, delivery_obj.gps_lng)

    # Phase: inward (always exists if roll exists)
    inward_data = {
    ...
    if delivery_obj:
        ...
        delivery_data = {
            ...
            # helpful for admin list (customer might be known only at dispatch)
...
# --------------------------------------------------------
# TIMELINE (Query param)
# /api/timeline/?qr=<encoded_qr>
# --------------------------------------------------------
@api_view(["GET"])
...
# --------------------------------------------------------
# TIMELINE FROM PREVIOUS QR (Query param)
# /api/timeline/previous/?qr=<previous_qr>
# --------------------------------------------------------
@api_view(["GET"])
...
# ========================================================
# ✅ ADMIN PANEL APIs (CEO/Manager Dashboard)
# ========================================================
...
# --------------------------------------------------------
# ✅ Flutter calls this:
# GET /api/admin/summary/?customer=...&limit=30
# --------------------------------------------------------
@api_view(["GET"])
def admin_summary(request):
    ...
    for j in active_jobs_qs:
        ...
        active_printing.append({
            "roll_id": r.roll_id,
            "qr_value": r.qr_value,          # important: used by Flutter to open timeline
    ...
    for dc in delivery_qs:
        ...
        # if customer filter is applied, skip other customers
...
# --------------------------------------------------------
# ✅ Active printing list (optional)
# GET /api/admin/active/?limit=50
# --------------------------------------------------------
@api_view(["GET"])
...
def admin_search(request):
    ...
    if q and not (customer or roll_id or job_id or status):
        ...
        root_qrs = set(resolve_root_qr(x) for x in dispatch_qrs if x)
        rolls_from_dispatch = RollMaster.objects.filter(qr_value__in=list(root_qrs)).values_list("id", flat=True) if root_qrs else []
